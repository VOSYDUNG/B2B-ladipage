...
const TRANSLATIONS = {
  "vi": {
    ...
    "products.title": "7 sản phẩm cộng dồn doanh số",
...
const PRODUCTS_DATA = [
  {
    ...
    packLo: 'ກັບ 45 ເມັດ',
    price: 69000,
    badgeVi: 'Thảo dược nổi bật',
    badgeLo: 'ສະຫມຸນໄພພົ້ນເດັ່ນ',
    formulation: 'Thảo dược',
    descVi: 'Thông tin nhận diện theo tài liệu NNC. Vui lòng liên hệ NNC để nhận tài liệu chuyên môn chính thức.',
    descLo: 'ຂໍ້​ມູນ​ຕາມ​ເອກະສານ NNC. ກະລຸນາຕິດຕໍ່ NNC ສຳລັບເອກະສານວິຊາການ.',
...
function handleFormSubmit(e) {
  ...
  if (window.db && window.addDoc && window.collection) {
    try {
      window.addDoc(window.collection(window.db, 'campaign_participants'), {
        participantId: participantId,
        fullname: registrationInfo.fullname,
        phone: registrationInfo.phone,
        businessName: registrationInfo.business,
        province: registrationInfo.province,
        referralCodeUsed: registrationInfo.refCode,
...
function spinWheel() {
  ...
  // The wheel is now inside the modal, so we do not hide the modal.
  const canvas = document.getElementById('lucky-wheel-canvas');
  if (!canvas) {
    isSpinning = false;
    return;
...
function openWhatsApp(intent, phoneOverride) {
  let headline = '';
  ...
  const targetPhone = phoneOverride || CONFIG.whatsappNumber;
